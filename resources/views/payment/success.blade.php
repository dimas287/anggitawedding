...
<div class="max-w-2xl mx-auto py-8 px-4">
    <div class="bg-white dark:bg-white/5 rounded-3xl shadow-xl p-8 border border-gray-100 dark:border-white/10 text-center">
        ...
        @if($paymentStatus === 'pending' && $isVA && $payCode)
            <div class="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-6 mb-6 text-left relative overflow-hidden shadow-inner">
                ...
                <div class="relative z-10">
                    ...
                    <div class="mt-4 border-t border-b border-gray-100 dark:border-white/10 py-5 mb-5">
                        @if($payCompanyCode)
                    ...
                    <div class="flex justify-between items-center bg-white dark:bg-[#1a1a1a] p-4 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm">
                        <div>
                        ...
                        <button type="button" onclick="copyText('{{ $cleanAmount }}', this)" class="text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white text-xs flex items-center px-3 py-1.5 bg-gray-50 dark:bg-white/5 rounded-lg border dark:border-white/10 focus:outline-none"><i class="far fa-copy mr-1"></i> Salin Nominal</button>
                    </div>

                    @if(isset($paymentDetails['pdf_url']))
                        <a href="{{ $paymentDetails['pdf_url'] }}" target="_blank" class="mt-5 block text-center text-sm font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:underline">
            ...
            @if(isset($paymentDetails['expiry_time']))
                <div class="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-100 dark:border-red-800/40 rounded-xl p-4 text-sm font-medium mb-6 flex items-center justify-center gap-2" id="expiry-timer" data-expiry="{{ $paymentDetails['expiry_time'] }}">
        ...
        <div class="bg-yellow-50 dark:bg-yellow-900/15 border border-yellow-200 dark:border-yellow-700/40 rounded-2xl p-6 mb-8 text-left space-y-3 text-sm">
            <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Booking Code</span><span class="font-bold text-yellow-700 dark:text-yellow-500">{{ $booking->booking_code }}</span></div>
            <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Pengantin</span><span class="font-semibold text-gray-800 dark:text-white">{{ $booking->groom_name }} & {{ $booking->bride_name }}</span></div>
            <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Tanggal Acara</span><span class="font-medium text-gray-800 dark:text-white">{{ $booking->event_date->isoFormat('D MMMM Y') }}</span></div>
            <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Paket</span><span class="font-medium text-gray-800 dark:text-white">{{ $booking->package->name }}</span></div>
            @if($payment)
                <div class="flex justify-between items-center pt-2 border-t border-yellow-200/50 dark:border-yellow-700/20 text-green-700 dark:text-green-400 font-bold text-base">
        ...
        <div class="bg-blue-50 dark:bg-blue-900/15 rounded-2xl p-5 text-sm text-blue-700 dark:text-blue-300 mb-8 border border-blue-100 dark:border-blue-800/20 leading-relaxed">
            <i class="fas fa-info-circle mr-1.5"></i> {{ $isInvitationOnly ? 'Detail pembayaran telah dicatat. Jika Anda memilih transfer manual, tunggu admin memverifikasi.' : 'Detail booking & jadwal konsultasi telah dikirim ke email Anda.' }}
        </div>

        <div class="flex flex-col gap-3">
            @if(in_array($paymentStatus, ['pending', 'failed']))
            <a href="{{ route('payment.checkout', $booking->id) }}?reset=1"
               class="block w-full gold-gradient text-white font-bold py-4 rounded-2xl text-sm hover:shadow-lg transition-all">
                <i class="fas fa-sync-alt mr-2"></i> Coba Lagi / Ganti Metode
            </a>
            <a href="{{ route('user.booking.show', $booking->id) }}"
               class="block w-full border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 font-semibold py-3.5 rounded-2xl text-sm hover:bg-gray-50 dark:hover:bg-white/5 transition-all">
            ...
            <a href="{{ route('user.invitation.index', $booking->id) }}"
               class="block w-full border-2 border-yellow-400 dark:border-yellow-500 text-yellow-600 dark:text-yellow-500 font-semibold py-4 rounded-2xl text-sm hover:bg-yellow-50 dark:hover:bg-yellow-900/10 transition-all">
