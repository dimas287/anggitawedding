    ...
    <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category') }}" placeholder="Contoh: Tips, Inspirasi, Dekorasi"
                ...
                <div>
                    ...
                    <select name="is_published" id="is_published" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition-all outline-none">
                        ...
                        <option value="1" {{ old('is_published') == '1' ? 'selected' : '' }}>Publish Sekarang</option>
                    </select>
                </div>

                {{-- Thumbnail --}}
                <div class="md:col-span-2">
                    <label for="thumbnail" class="block text-sm font-semibold text-gray-700 mb-2">Gambar Thumbnail Halaman Depan</label>
                    <div class="flex items-center gap-4">
                        <input type="file" name="thumbnail" id="thumbnail" accept="image/*"
                               class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 cursor-pointer text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100">
                    </div>
                    <p class="text-[10px] text-gray-400 mt-2 italic">Format: JPG, PNG, WEBP. Maks 2MB. Disarankan ratio 16:9.</p>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="2"
                          ...
                          placeholder="Ringkasan singkat maksimal 200 karakter...">{{ old('excerpt') }}</textarea>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title') }}" placeholder="Judul yang tampil di Google"
                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition-all outline-none">
                </div>
                <div>
                    ...
                    <textarea name="meta_description" id="meta_description" rows="3"
                               class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition-all outline-none resize-none"
                               placeholder="Deskripsi singkat untuk hasil pencarian Google...">{{ old('meta_description') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4">
            <button type="submit" class="gold-gradient text-white px-10 py-4 rounded-xl font-bold font-poppins shadow-lg hover:opacity-90 transition-all flex items-center gap-2">
                <i class="fas fa-paper-plane"></i> Simpan & Publish Artikel
            </button>
        </div>
    </form>
</div>
