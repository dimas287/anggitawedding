        ...
        <div class="space-y-4 mb-14" x-data="{ open: null }">
            ...
            <div class="bg-white dark:bg-gray-800/50 dark:border dark:border-white/5 rounded-2xl shadow-sm overflow-hidden">
                <button @click="open = open === {{ $i }} ? null : {{ $i }}"
                    ...
                    <span class="font-semibold text-gray-800 dark:text-gray-200 text-sm leading-relaxed">{{ $faq->question }}</span>
                ...
                <div x-show="open === {{ $i }}" x-collapse class="px-6 pb-6">
                    <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed border-t border-gray-100 dark:border-white/10 pt-4">{{ $faq->answer }}</p>
                </div>
            </div>
            @endforeach
        </div>
